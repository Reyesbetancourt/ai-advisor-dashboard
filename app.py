...
@st.cache_data
def load_data():
    series = pd.read_csv("data/series_mensual.csv", parse_dates=["mes"])
    casos = pd.read_csv("data/casos_uso.csv")
    stack = pd.read_csv("data/stack_ia.csv")

    # Limpieza / tipado defensivo
    series = series.dropna(subset=["mes", "vertical"])
    series["adopcion_ia"] = series["adopcion_ia"].astype(bool)
    for c in ["horas_manuales", "horas_ahorradas", "ingresos_mxn",
              "pct_automatizado", "nps"]:
        series[c] = pd.to_numeric(series[c], errors="coerce")
    series["clientes_activos"] = series["clientes_activos"].astype(int)
